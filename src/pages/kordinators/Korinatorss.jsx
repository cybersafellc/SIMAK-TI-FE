...
export default function Kordinatorss() {
    ...
    try {
      ...
      const [mahasiswa] = await Promise.all([
        new Promise((resolve, reject) => {
          getKordinatorsSearch(target.value, accessToken, (err, data) => {
            if (err) reject(err);
            else resolve(data);
          });
        }),
  ...
  const navigateIncrement = () => {
    window.location.href =
      "/kordinators/kordinators?page=" + (parseInt(page) + 1);
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [mahasiswa] = await Promise.all([
          new Promise((resolve, reject) => {
            getKordinators(query.get("page") || 1, accessToken, (err, data) => {
              if (err) reject(err);
              else resolve(data);
            });
          }),
            ...
            <div className="container">
              <div className="row">
                <div className="col-12">
                  <h1 className="h4">Kordinators</h1>
            ...
            <Container>
              <div>
                <Form.Group className="mb-3" controlId="formBasicEmail">
                  <Form.Label>Cari Kordinators</Form.Label>
                  <Form.Control
                    type="text"
                  ...
                  <Form.Text className="text-muted text-capitalize">
                    Untuk Mencari Kordinator, Silahkan Masukkan nidn atau nama
                    kordinator
                  </Form.Text>
                </Form.Group>
              </div>
