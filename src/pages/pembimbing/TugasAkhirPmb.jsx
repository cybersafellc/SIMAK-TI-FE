...
export default function TugasAkhirPmb() {
  const handleDetails = async (id) => {
    window.location.href = `/pembimbings/tugas-akhir/details?id=${id}`;
    return;
  };
  ...
  const navigateIncrement = () => {
    window.location.href =
      "/pembimbings/tugas-akhir?page=" + (parseInt(page) + 1);
  };
    ...
    try {
      ...
      const [mahasiswa] = await Promise.all([
        new Promise((resolve, reject) => {
          getTASearch(target.value, accessToken, (err, data) => {
            if (err) reject(err);
            else resolve(data);
          });
        }),
  ...
  React.useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [kpData, pembimbing1] = await Promise.all([
          new Promise((resolve, reject) => {
            getTA(query.get("page") || 1, accessToken, (err, data) => {
              if (err) reject(err);
              else resolve(data);
            });
          }),
              ...
              <div className="container">
                <div className="row">
                  <div className="col-12">
                    <h1 className="h4">Bimbingan Tugas Akhir</h1>
                    ...
                    <div className="card widget-card border-light shadow-sm h-100">
                      <div className="card-body p-4">
                        ...
                        <div>
                          ...
                          >
                            <Form.Label>
                              Cari Mahasiswa Bimbingan TA dengan Anda
                            </Form.Label>
                            <Form.Control
                              type="text"
                              placeholder="Nama"
                            ...
                            <Form.Text className="text-muted text-capitalize">
                              Untuk Mencari Informasi Mahasiswa Bimbingan TA,
                            ...
                            <tbody>
                              {kp?.map((data, index) => (
                                <tr>
                                  ...
                                  <td>
                                    {data?.details_pengajuan_ta?.status ===
                                      "ditolak" && (
